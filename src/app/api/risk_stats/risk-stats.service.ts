...
export class RiskStatsService {
  ...
  create(risk_stats: IRiskStatsBase): Observable<IRiskStats> {
    this.setReqOptions();
    return this.http
      .post('/api/risk_stats', JSON.stringify(risk_stats), this.req_options)
  ...
  read(createdAt: string | 'latest' | Date): Observable<IRiskStats> {
    return this.http
      .get(`/api/risk_stats/${createdAt}`, new RequestOptions({
        headers: new Headers({
  ...
  update(prevRecord: IRiskStats, newRecord: IRiskStatsBase): Observable<IRiskStats> {
    this.setReqOptions();
    return this.http
      .put(`/api/risk_stats/${prevRecord.createdAt}`, JSON.stringify(newRecord), this.req_options)
      .map((r: Response) => r.json() as IRiskStats)
      .catch(handleError)
  }
  ...
  destroy(createdAt: string | Date): Observable<{}> {
    this.setReqOptions();
    return this.http
      .delete(`/api/risk_stats/${createdAt}`, this.req_options)
